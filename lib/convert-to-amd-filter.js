...
module.exports = class ConvertToAMD extends Filter {
  constructor(inputTree, options = {}) {
    ...
    this.loadingFilePath = options.loadingFilePath;
    ...
    // The index file could contain additional scripts and inline code
    // that need to be loaded at the end of the AMD loading process
    // This is collected when processing the index file.
    this.scriptsToLoad = [];
  }
  ...
  processString(code, relativePath) {
    ...
    if (relativePath.indexOf('index.html') >= 0) {
      return this._processIndexFile(code);
    }

    return code;
  }

  _processIndexFile(code) {
    ...
    if (inlineScripts.length > 0) {
      let afterLoadingScript = replaceRequireAndDefine(inlineScripts.join('\n\n'));
      fs.writeFileSync(path.join(this.outputPath, 'assets/after-amd-loading.js'), beautify_js(afterLoadingScript, {
        indent_size: 2,
        max_preserve_newlines: 1
      }));
      scriptsToLoad.push('"assets/after-amd-loading.js"');
    }
    ...
    const amdScripts = [
      ...
      `<script src="${this.loadingFilePath}" data-amd-loading=true></script>`
    ];
  ...
  async build() {
    ...
    const scripts = moduleInfos.scripts;
    ...
    if (names !== moduleInfos.names || scripts !== moduleInfos.scripts) {
      let loadingScript = amdLoadingTemplate(moduleInfos);

      fs.writeFileSync(path.join(this.outputPath, this.loadingFilePath), beautify_js(loadingScript, {
        indent_size: 2,
        max_preserve_newlines: 1
      }));
    }
