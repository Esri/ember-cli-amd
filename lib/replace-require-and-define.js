...
// Use Espree to parse the code and eswalk to walk thru the code
  ...
  eswalk(ast, function(node) {
    ...
    switch (node.type) {
      case 'CallExpression':
        ...
        // Collect the external AMD modules
        ...
        if (node.callee.name === 'eval' && node.arguments[0].type === 'Literal' && typeof node.arguments[0].value === 'string') {
          ...
          const evalCodeAfter = replaceRequireAndDefine(evalCode, amdPackages, externalAmdModules);
